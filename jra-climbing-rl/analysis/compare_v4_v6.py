...
print("Evaluating climb_ppo_v4 (300K timesteps)...")
summary_v4 = report("v4 (300K)", run_eval("outputs/trained_models/climb_ppo_v4", difficulty=0.0))

print("\nEvaluating climb_ppo_v6 (1M timesteps, same config)...")
summary_v6 = report("v6 (1M)", run_eval("outputs/trained_models/climb_ppo_v6", difficulty=0.0))

print("\n" + "=" * 60)
print("COMPARISON — same reward, same difficulty, different training length")
print("=" * 60)
print(f"{'Metric':<28} {'v4 (300K)':<16} {'v6 (1M)':<16}")
print(f"{'Avg episode length':<28} {summary_v4['avg_steps']:<16.1f} {summary_v6['avg_steps']:<16.1f}")
print(f"{'Pose error (lower=better)':<28} {summary_v4['pose_error_mean']:<16.3f} {summary_v6['pose_error_mean']:<16.3f}")
print(f"{'Smoothness (lower=better)':<28} {summary_v4['smoothness_mean']:<16.4f} {summary_v6['smoothness_mean']:<16.4f}")
print(f"{'Success rate':<28} {summary_v4['success_rate']:<16.1%} {summary_v6['success_rate']:<16.1%}")